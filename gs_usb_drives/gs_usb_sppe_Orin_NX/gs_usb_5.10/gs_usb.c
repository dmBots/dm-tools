...
// 		// const u32 ts = gs_usb_set_timestamp(dev, skb, hf);			/*原始值*/
// 		// (void)gs_usb_set_timestamp(dev, skb, hf);					/*新增值*/
// 		can_rx_offload_queue_tail()										/*new*/

// 		// rc = can_rx_offload_queue_timestamp(offload, skb, ts);		/*old*/
// 		can_rx_offload_get_echo_skb_queue_tail()						/*new*/
...
// }
{
    ...
    rc = can_rx_offload_queue_tail(&dev->offload, skb);
...
// }
{
    ...
    // return can_rx_offload_get_echo_skb_queue_tail(
    //         &dev->offload, hf->echo_id, NULL);
	return can_rx_offload_get_echo_skb_queue_timestamp(
        &dev->offload, hf->echo_id, 0, NULL);
}
...
static void gs_usb_receive_bulk_callback(struct urb *urb)
{
	...
	if (hf->flags & GS_CAN_FLAG_OVERFLOW) {
		...
		// cf->len = CAN_ERR_DLC;		/*原始值*/
		cf->can_dlc = CAN_ERR_DLC;		/*新增值*/
				     ...
				     struct net_device *netdev)
{
	...
	if (can_is_canfd_skb(skb)) {
		...
		// memcpy(hf->classic_can->data, cf->data, cf->len);					/*原始值*/
		memcpy(hf->classic_can->data, cf->data,can_dlc2len(cf->can_dlc));		/*新增值*/
	}
	...
	// can_put_echo_skb(skb, netdev, idx, 0);	/*原始值*/
	can_put_echo_skb(skb, netdev, idx);			/*新增值*/
	...
	if (unlikely(rc)) {			/* usb send failed */
		...
		// can_free_echo_skb(netdev, idx, NULL);	/*原始值*/
		can_free_echo_skb(netdev, idx);				/*新增值*/
		gs_free_tx_context(txc);
...
static const struct net_device_ops gs_usb_netdev_ops = {
	...
	// .ndo_eth_ioctl = gs_can_eth_ioctl,	/*原始值*/
	.ndo_do_ioctl = gs_can_eth_ioctl,		/*新增值*/
};
...
static int gs_usb_get_ts_info(struct net_device *netdev,
                              struct ethtool_ts_info *info)
{
    /* Jetson 5.10：只支持 software timestamp */
    info->so_timestamping =
        SOF_TIMESTAMPING_RX_SOFTWARE |
        SOF_TIMESTAMPING_TX_SOFTWARE;
