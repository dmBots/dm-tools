# gs_usb_sppe_Orin_NX使用说明

> - ‘sppe’ 是单词‘special purpose’，当然这不是一个严谨的做法，不过表明了这个驱动工具包只能适用于Jetson Orin系列板卡，如Orin Nano、Orin NX之类
> - 每个子文件夹中只包含`gs_usb.c` `Makefile` `compat_5_xx.h`以及已生成的`gs_usb.ko`文件，必要时可能需要您自行编译一次，来替换掉现在的.ko文件实现更好的适配
...
- 不同内核版本的gs_usb配置不同，因此不可以跨内核、跨平台使用（如：RK3588系列板卡）
- 你可以参考下表来选择内核版本
