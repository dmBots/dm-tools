# USB2CANFD SDK / C++ / arm 平台流程

## 环境与默认行为
- 原 README 的测试环境为 Orin Nano 上的 Ubuntu 20.04 / 22.04。
...
- 控制命令仍通过 `control_mit(...)` 发送。
- 电机反馈通过 `Get_Position()`、`Get_Velocity()`、`Get_tau()`、`getTimeInterval()` 读取。
- 1kHz 控制循环沿用原 README 推荐的 `std::this_thread::sleep_until(...)` 节奏控制。
